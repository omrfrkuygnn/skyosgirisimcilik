...
/// <summary>
/// Applies the schema on startup. SQL Server uses EF migrations. SQLite dev files use
/// <see cref="DatabaseFacade.EnsureCreated"/> plus targeted schema patches for backoffice tables.
/// </summary>
public static class DatabaseInitializer
{
    ...
    public static async Task InitializeAsync(IServiceProvider services, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                logger.LogInformation("SQLite schema ensured for provider {Provider}.", context.Database.ProviderName);

                await PatchPartnerColumnsAsync(context, cancellationToken).ConfigureAwait(false);
                await SeedPartnersAsync(context, cancellationToken).ConfigureAwait(false);
                await SqliteDevSchemaEnsurer.EnsureBackofficeTablesAsync(context, logger, cancellationToken).ConfigureAwait(false);
            }
    ...
    private static async Task PatchPartnerColumnsAsync(SkyOSDbContext context, CancellationToken cancellationToken)
    {
        if (!await TableExistsAsync(context, "Partners", cancellationToken).ConfigureAwait(false))
        {
            return;
        }
    ...
    private static async Task SeedPartnersAsync(SkyOSDbContext context, CancellationToken cancellationToken)
    {
        ...
        foreach (var seedPartner in seedPartners)
        {
            var existing = await context.Partners
                .FirstOrDefaultAsync(p => p.Id == seedPartner.Id || p.Name == seedPartner.Name, cancellationToken)
            ...
            {
                existing.LogoUrl = seedPartner.LogoUrl;
                existing.Description = seedPartner.Description;
                existing.Address = seedPartner.Address;
                existing.Phone = seedPartner.Phone;
                existing.WebsiteUrl = seedPartner.WebsiteUrl;
                existing.SupportLetterUrl = seedPartner.SupportLetterUrl;
                existing.IsVerifiedRelationship = seedPartner.IsVerifiedRelationship;
                existing.DisplayOrder = seedPartner.DisplayOrder;
            }
        }
